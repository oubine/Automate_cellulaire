...
void Window_Dim1::launchSimulation() {//méthode pour lancer la simulation
    // création de l'état
...
void Window_Dim2::launchSimulation() {//méthode pour lancer la simulation
    // création de l'état
    Etat2D e(dimension,1);
    // on récupère les données de l'état de l'interface graphique pour que ça corresponde à l'objet qu'on vient de créer
    for(unsigned int counter = 0; counter < dimension; ++counter) {
        for(unsigned int counter2 =0; counter<dimension; ++counter2)
        {
